...
if ( ! function_exists( 'plumco_dynamic_styles' ) ) {
  function plumco_dynamic_styles() {

    // Typography
    $plumco_get_typography  = plumco_get_typography();
    ...
    // Logo
    $plumco_logo_top     = cs_get_option( 'plumco_logo_top' );
    $plumco_logo_bottom  = cs_get_option( 'plumco_logo_bottom' );
...
$layout_boxed = ( $bg_pattern === 'custom-pattern' ) ? 'background-image: url('. esc_url($custom_bg_pattern) .');' : 'background-image: url('. esc_url(PLUMCO_IMAGES) . '/patterns/'. $bg_pattern .'.png);';
?>
  .layout-boxed {
    <?php echo   wp_kses_data( $layout_boxed ); ?>
  }
...
if ($plumco_meta) {
  $transparent_header = $plumco_meta['transparency_header'];
  $transparent_menu_color = $plumco_meta['transparent_menu_color'];
  $transparent_menu_hover = $plumco_meta['transparent_menu_hover_color'];
} else {
  $transparent_header = '';
...
if ($breadcrumbs_text_color) {?>
  ...
  .wpo-page-title-section .breadcrumb li {
...
<?php
  ...
  echo   wp_kses_data( $plumco_get_typography );
  $output = ob_get_clean();
  return $output;
 ...
 * Custom Font Family
 */
if ( ! function_exists( 'plumco_custom_font_load' ) ) {
  function plumco_custom_font_load() {
...
/* Custom Styles */
if( ! function_exists( 'plumco_custom_css' ) ) {
  function plumco_custom_css() {
    wp_enqueue_style('plumco-default-style', get_template_directory_uri() . '/style.css');
    $output  = plumco_custom_font_load();
    $output .= plumco_dynamic_styles();

    if( function_exists( 'plumco_compress_css_lines' ) ) {
      $custom_css = plumco_compress_css_lines( $output );
       wp_add_inline_style( 'plumco-default-style', $custom_css );
    }
    plumco_typography_fonts();
  }
  add_action( 'wp_enqueue_scripts', 'plumco_custom_css' );
}

/* Custom JS */
if( ! function_exists( 'plumco_custom_js' ) ) {
  function plumco_custom_js() {
    $output = cs_get_option( 'theme_custom_js' );
    wp_add_inline_script( 'jquery-migrate', $output );
  }
  add_action( 'wp_enqueue_scripts', 'plumco_custom_js' );
}